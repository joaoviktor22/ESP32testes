...
const mb_parameter_descriptor_t device_parameters[] = {
    ...
    { CID_DEV_REG0, STR("MB_hold_reg-0"), STR("Data"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 189, 1,
                    0, PARAM_TYPE_U16, 2, OPTS( 0,0,0 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_DEV_REG1, STR("MB_hold_reg-1"), STR("Data"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 0, 1,
                    0, PARAM_TYPE_U16, 2, OPTS( 0,0,0 ), PAR_PERMS_READ_WRITE_TRIGGER }
};
...
static esp_err_t master_init(void)
{
    ...
    ESP_LOGI(MASTER_TAG, "Modbus master stack initialized...3");
    ...
    MASTER_CHECK((err == ESP_OK), ESP_ERR_INVALID_STATE,
                                ...
                                (uint32_t)err);
    ESP_LOGI(MASTER_TAG, "Modbus master stack initialized...2");
    return err;
}
...
void app_main(void)
{
    ...
    ESP_LOGI(MASTER_TAG, "Modbus master stack initialized...1");
    ESP_ERROR_CHECK(master_init());
    ESP_LOGI(MASTER_TAG, "Modbus master stack initialized...1");
    vTaskDelay(10);
    
    // Write registers to predefined state
    uint16_t register_data = 0x5678;
    write_modbus_parameter(CID_DEV_REG0, &register_data);
    register_data = 0x1234;
