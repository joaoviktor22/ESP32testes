...
//static const esp_bt_io_cap_t esp_bt_io_cap = ESP_BT_IO_CAP_IN;
...
static int receive_JSON(esp_spp_cb_param_t *param)
{
    // data pattern {"SSID":"PUT_YOUR_WIFI_NAME", "PSK":"PUT_YOUR_WIFI_PASS"}
    char *incoming_data = (char *)param->data_ind.data;
    ...
    ESP_LOGI(SPP_TAG, "WiFi Credentials OTA SSID:(%s) & PSK: (%s)\n",wifi_ssid,wifi_psk);
    //esp_spp_write(param->write.handle, strlen(wifi_psk), (uint8_t *)wifi_psk);
    //esp_spp_write(param->write.handle, strlen(incoming_data), (uint8_t *)incoming_data);
    return 0;
}

static int send_JSON(esp_spp_cb_param_t *param)
{
    cJSON *root = NULL;
    cJSON *fmt = NULL;
    ...
    char *out = NULL;
    out = cJSON_Print(root);//transforma em string
    esp_spp_write(param->write.handle, strlen(out), (uint8_t *)out);
    cJSON_Delete(root);
    return 0;
}
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        if (param->data_ind.len < 128) {
            ...
            // param->data_ind.data[param->data_ind.len-2] = '\0';
    ...
    case ESP_SPP_SRV_OPEN_EVT:
        ESP_LOGI(SPP_TAG, "ESP_SPP_SRV_OPENn_EVT status:%d handle:%d, rem_bda:[%s]", param->srv_open.status,
