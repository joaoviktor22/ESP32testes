...
static int receive_JSON(esp_spp_cb_param_t *param)
{
    ...
    ESP_LOGI(SPP_TAG, "WiFi Credentials SSID:(%s) & PSK: (%s)\n",wifi_ssid,wifi_psk);
    ota_update();
    return 0;
}
...
void disconnect_bt(){
    int ret;
    printf("Tentativa de Desconectar bt0");
    //vTaskDelay(500 / portTICK_PERIOD_MS;);
    if ((ret = esp_bluedroid_disable()) != ESP_OK){
        ESP_LOGE(SPP_TAG, "%s  disable bluedroid failed: %s\n", __func__, esp_err_to_name(ret));
    }
    printf("Tentativa de Desconectar bt1");
    if ((ret = esp_bluedroid_deinit()) != ESP_OK) {
        ESP_LOGE(SPP_TAG, "%s  deinit bluedroid failed: %s\n", __func__, esp_err_to_name(ret));
    }printf("Tentativa de Desconectar bt2");
    if ((ret = esp_bt_controller_disable()) != ESP_OK) {
        ESP_LOGE(SPP_TAG, "%s  disable controller failed: %s\n", __func__, esp_err_to_name(ret));
    }printf("Tentativa de Desconectar bt3");
    if ((ret = esp_bt_controller_deinit()) != ESP_OK) {
        ESP_LOGE(SPP_TAG, "%s  deinit controlled failed: %s\n", __func__, esp_err_to_name(ret));
    }
    printf("Disconectado bt");
}